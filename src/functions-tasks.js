...
function getPolynom(...args) {
  ...
  return (inputValue) =>
    args.reduceRight(
      (result, coeff, index, coefficients) =>
        result + coeff * inputValue ** (coefficients.length - index - 1),
      0
    );
}
  ...
  return () => {
    ...
    while (count < attempts) {
      try {
        ...
        count += 1;
        if (count >= attempts) throw new Error(err);
      }
    }
    return null;
  };
...
function logger(func, logFunc) {
  return (...args) => {
    ...
    const funcCall = `${func.name}(${argsStr})`;
...
function getIdGeneratorFunction(startFrom) {
  const counter = { currentId: startFrom };
  return () => {
    const returnId = counter.currentId;
    counter.currentId += 1;
    return returnId;
  };
}
